...
if not networkaction.startNode(server=True, client=False):
...
class RpcServer(SimpleXMLRPCServer):
    def __init__(self, bind_address, bind_port):
        # Create server
        self.server = SimpleXMLRPCServer((bind_address, bind_port),requestHandler=RequestHandler, allow_none=True)
        ...
        # shutdown rpc server
        self.server.register_function(lambda: os.kill(os.getpid(), signal.SIGTERM), 'stop')
